...
def freq(data):
    ...
    # Loop through data, compare the current value to every other value. If we find a match, increment count.
    # Then set freq of the current number to count, reset count and move on to the next number
    for num in data:
        for compNum in data:
            if num == compNum:
                count += 1
